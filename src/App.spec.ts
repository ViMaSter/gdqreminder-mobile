...
const timings = [
  ["after AGDQ2024, shows no runs, can show upcoming event", "2024-03-15%2000:00:00", "WEB"],
  ["during AGDQ2024, shows current runs, can show upcoming event", "2024-01-15%2000:00:00", "AGDQ"]
]
...
timings.forEach(([testName, date, title]) => {
  test(testName, async ({ page }) => {
    ...
    await expect(page.locator('mwc-top-app-bar-fixed [slot="title"]')).toHaveText(new RegExp(title));
