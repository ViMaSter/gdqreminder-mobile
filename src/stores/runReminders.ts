...
const reflectInFirestore = async (runs: string[]) => {
  ...
  if (!currentUserID) {
    console.warn(
      `Local save successful, but failed to update run reminder ${runs} to firestore; no current user. The user sign should have completed by now.`,
    );
    return;
  }
  ...
  actions: {
    async add(
      ...
      if (calendarError.error) {
        console.error(
          "Failed to add run reminder to calendar:" + calendarError.error,
        );
        return false;
      }
    ...
    async remove(pk: string): Promise<boolean> {
      ...
      const calendarError = await Calendar.cleanupEvents({
        sync_ids: Array.from(this.$state.runs),
      });
      if (calendarError.error) {
        console.error(
          "Failed to remove run reminder from calendar:" + calendarError.error,
        );
        return false;
      }
